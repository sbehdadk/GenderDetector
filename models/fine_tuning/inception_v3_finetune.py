...
class MobileNetDeepEstimator:
    ...
    def __call__(self):
        ...
        image_generator_imdb, train_keys_imdb, val_keys_imdb = self.__preprocessor__imdb()
        # we need to recompile the model for these modifications to take effect
        # we use SGD with a low learning rate
        model.compile(optimizer=SGD(lr=self.learning_rate/10, momentum=0.9),
        ...
        model_checkpoint = ModelCheckpoint(
                    os.path.join('checkpoints/fine_tune', 'val_inceptionv3_imdb_128_10Epoch_Adam.{epoch:02d}-{val_loss:.2f}.hdf5'),
                    monitor="val_loss",
                    verbose=1,
                    save_best_only=True,
                    mode="auto",
